...
public class MainView extends VerticalLayout {
    ...
    public MainView(BridgeWorkflowService workflow) {
        ...
        setSizeFull(); setMaxWidth("1500px"); getStyle().set("margin", "0 auto");
        add(new H1("Offline Worklog Bridge"));
        add(new Paragraph("Capture detailed worklogs in one network context, review the accounting transformation locally, then publish an aggregated timesheet after switching to a separate network context."));
        ...
        configureSourceGrid(); configureAllocationGrid(); configureDestinationGrid();
        commentPreview.setWidthFull(); commentPreview.setReadOnly(true); commentPreview.setMinHeight("110px");
        add(new Paragraph("1. Captured source worklogs"), sourceGrid, new Paragraph("2. Allocation preview"), allocationGrid,
                new Paragraph("3. Destination entries"), destinationGrid, commentPreview);
        expand(sourceGrid, allocationGrid, destinationGrid);
        workflow.latestSnapshotId().ifPresent(id -> { snapshotId = id; refresh(); });
    }
    ...
    private void configureAllocationGrid() {
        ...
        allocationGrid.addColumn(d -> d.tasks().stream().map(t -> t.issueKey() + "=" + format(t.destinationMinutes())).reduce((a, b) -> a + ", " + b).orElse("BLOCKED"))
    ...
    private void configureDestinationGrid() {
        ...
        destinationGrid.addSelectionListener(event -> event.getFirstSelectedItem().ifPresent(entry -> commentPreview.setValue(entry.comment())));
    }

    private void captureWeek() {
        LocalDate start = LocalDate.now().with(TemporalAdjusters.previousOrSame(DayOfWeek.MONDAY));
        snapshotId = workflow.capture(start, start.plusDays(6)); refresh();
        Notification.show("Snapshot captured. Disconnect the source network before publishing in a real deployment.");
    }

    private void publish() {
        if (snapshotId == null) { Notification.show("Capture a snapshot first"); return; }
        try {
        ...
        } catch (RuntimeException ex) { Notification.show(ex.getMessage()); }
    }

    private void refresh() {
        sourceGrid.setItems(workflow.captured(snapshotId)); allocationGrid.setItems(workflow.plan(snapshotId));
        try {
            var preview = workflow.destinationPreview(snapshotId); destinationGrid.setItems(preview);
            preview.stream().findFirst().ifPresent(entry -> commentPreview.setValue(entry.comment()));
        } catch (RuntimeException ex) {
            destinationGrid.setItems(java.util.List.of()); commentPreview.setValue(ex.getMessage());
        }
    }

    private static String format(int minutes) { return "%dh %02dm".formatted(minutes / 60, minutes % 60); }
}
